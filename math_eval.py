...
def main(llm, tokenizer, data_name, args):
    ...
    for epoch in range(max_func_call):
        ...
        for k in range(len(remain_prompts)):
            ...
            # 在达到最大轮数时，拼接执行结果后再进行一次额外生成，并追加收尾提示词引导模型产出最终答案
        # if epoch == max_func_call - 1 and len(remain_prompts) > 0:
        #     hint = "\nThe final answer is:\n\n\\[\n\\boxed{"
        #     extra_prompts = [query + hint for (_, query) in remain_prompts]
        #     if args.use_vllm:
        #         extra_outputs = llm.generate(
        #             extra_prompts,
        #             SamplingParams(
        #                 temperature=args.temperature,
        #                 top_p=args.top_p,
        ...
        #             ),
        #         )
        #         extra_outputs = sorted(extra_outputs, key=lambda x: int(x.request_id))
        #         extra_outputs = [o.outputs[0].text for o in extra_outputs]
        #     else:
        #         extra_outputs = generate_completions(
        #             model=llm,
        #             tokenizer=tokenizer,
        ...
        #             stop_id_sequences=stop_words,
        #         )
        #     for (i, query), out in zip(remain_prompts, extra_outputs):
        #         end_prompts.append((i, (query + hint + out.rstrip())))
        #     remain_prompts = []
