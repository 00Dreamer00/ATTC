...
def setup(args):
    ...
    if args.use_vllm:
        ...
        llm = LLM(model=args.model_name_or_path, tensor_parallel_size=1, trust_remote_code=True)
        tokenizer = None
    else:
    ...
    # 加载 PRM模型
    prm_device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")

    prm_model_path = '/public/home/ljt/xrt/model/Universal-PRM-7B'
    ...
    for data_name in data_list:
        ...
        results.append(main(llm, tokenizer, data_name, args, prm_model, prm_tokenizer, prm_device))
...
def main(llm, tokenizer, data_name, ags, prm_model, prm_tokenizer, prm_device):
    ...
    for epoch in range(max_func_call):
        ...
        for k in range(len(remain_prompts)):
            ...
            query_for_score = query.split("```python")[0]
            steps = []
            steps.append(query_for_score)

            question = samples[i // args.n_sampling]['question']
            ...
            if not report == "Done":
                ...
                query += "The result shown above is correct\n"
            else:
                query += "My previous thinking seems to be incorrect. Let's think step by step."


            # if report == "Done":
            #     query += "The result shown above is correct."
            # if report == "Done":
            #     query += "I must trust the result shown above."
            # else:
            #     query +="The above results seem to be problematic.Let's think about it again."
